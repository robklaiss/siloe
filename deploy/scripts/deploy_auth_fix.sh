...
echo "Deploying authentication fix to $REMOTE_USER@$REMOTE_HOST:$REMOTE_DIR"

# 1. Create necessary directories on remote server
echo "Creating directories on remote server..."
ssh $SSH_OPTS $REMOTE_USER@$REMOTE_HOST "mkdir -p $REMOTE_DIR/app/Controllers $REMOTE_DIR/database/migrations"

# 2. Copy AuthController.php to the server
echo "Copying AuthController.php..."
scp $SCP_OPTS $LOCAL_DIR/app/Controllers/AuthController.php $REMOTE_USER@$REMOTE_HOST:$REMOTE_DIR/app/Controllers/

# 3. Create database migration script
echo "Creating database migration script..."
cat > /tmp/restore_auth_tables.php << 'EOL'
<?php
...
scp $SCP_OPTS /tmp/restore_auth_tables.php $REMOTE_USER@$REMOTE_HOST:$REMOTE_DIR/database/migrations/restore_auth_tables.php

# 4. Create restore script
echo "Creating restore script..."
cat > /tmp/restore_auth.sh << 'EOL'
#!/bin/bash
...
scp $SCP_OPTS /tmp/restore_auth.sh $REMOTE_USER@$REMOTE_HOST:$REMOTE_DIR/restore_auth.sh

# 5. Run the restore script on the server
echo "Running restore script on server..."
ssh $SSH_OPTS $REMOTE_USER@$REMOTE_HOST "cd $REMOTE_DIR && chmod +x restore_auth.sh && ./restore_auth.sh"
